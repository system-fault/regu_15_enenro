...
% Fecha:        1/11/2024
% Asignatura:   Regulacion Automatica
% Correo:       dmartinez076@ikasle.ehu.eus
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Comprobacion polos deseados en el LGR
...
hold on;
sgrid(xi,Wn);
xline(0, '--k', 'LineWidth', 0.5); % Marca la linea de cero
plot([real(Pd1),real(Pd2)],[imag(Pd1),imag(Pd2)],'r*');
xlabel('Parte Real');
ylabel('Parte Imaginaria');
...
plot([ts ts],[0 2],'k--');   % Linea de que marca ts

% Variables para la simulacion de simulink

final_escalon = 1;

KGH_controlado = G_adelanto_sin_K*G*H;

figure('Name','LGR sistema controlado'); 
rlocus(KGH_controlado);
axis ([-10 5 -8 8]);
...
xlabel('Parte Real');
ylabel('Parte Imaginaria');
title('Lugar de las raices sistema controlado');
hold off;
